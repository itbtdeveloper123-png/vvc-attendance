...
export const testConvertApiKeyRealtime = async (apiKey: string): Promise<{
  success: boolean;
  httpCode: number;
  status: 'active' | 'invalid' | 'error';
  latencyMs: number;
  ...
  if (!trimmed) {
    return {
      ...
      status: 'invalid',
      latencyMs: 0,
      message: 'ConvertAPI Secret Key មិនអាចទទេបានឡើយ',
    };
  }

  const start = performance.now();
  try {
    const res = await fetch(`https://v2.convertapi.com/user?Secret=${encodeURIComponent(trimmed)}`, {
      method: 'GET',
      headers: {
        'Accept': 'application/json',
      },
    ...
    if (!res.ok || (data && data.Active === false)) {
      return {
        ...
        status: 'invalid',
        latencyMs: latency,
        message: data?.Message || `ConvertAPI Auth Error (${res.status})`,
        detail: data?.Code ? `Error Code: ${data.Code}` : 'Invalid API Secret',
      };
    }

    const secondsLeft = data.SecondsLeft ?? 0;
    const userName = data.UserName || data.Email || 'ConvertAPI User';
    ...
    return {
      ...
      status: 'active',
      latencyMs: latency,
      message: `Key ត្រឹមត្រូវ និងដំណើរការល្អ! (${secondsLeft.toLocaleString()} Seconds Left / ម្ចាស់: ${userName})`,
      credits: secondsLeft,
      userName,
    ...
    return {
      ...
      status: 'error',
      latencyMs: Math.round(performance.now() - start),
      message: `មិនអាចភ្ជាប់ទៅ ConvertAPI បានឡើយ: ${err?.message || err}`,
    };
  }
};
...
export const TokensPage: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'active_sessions' | 'global_settings' | 'remove_bg_keys' | 'cutout_pro_keys' | 'gemini_keys' | 'ilovepdf_keys' | 'cloudconvert_keys' | 'convertapi_keys'>('active_sessions');
  ...
  // AI API Keys Management State (Remove.bg, Cutout.pro, Google Gemini, iLovePDF, CloudConvert & ConvertAPI)
  const currentServiceName = activeTab === 'cutout_pro_keys' ? 'cutout_pro' : activeTab === 'gemini_keys' ? 'gemini' : activeTab === 'ilovepdf_keys' ? 'ilovepdf' : activeTab === 'cloudconvert_keys' ? 'cloudconvert' : activeTab === 'convertapi_keys' ? 'convertapi' : 'remove_bg';
  ...
  const [convertApiCount, setConvertApiCount] = useState<number>(0);
  ...
  const loadCounts = async () => {
    try {
      const [rmbg, cutout, gemini, ilovepdf, cloudconvert, convertapi] = await Promise.all([
        ...
        adminApi.getApiKeys('convertapi'),
      ]);
      ...
      if (convertapi && convertapi.success) setConvertApiCount(convertapi.keys?.length || 0);
    } catch (e) {
      console.error(e);
      setGeminiCount(1);
    }
  ...
  const loadApiKeys = async (service: string = currentServiceName, silent = false) => {
    ...
    try {
      ...
      if (res && res.success && res.keys && res.keys.length > 0) {
        ...
        } else if (service === 'convertapi') {
          setConvertApiCount(res.keys.length);
        }
      } else if (service === 'gemini') {
  ...
  const { lastSyncTime, isSyncing, refreshNow } = useRealtimeSync({
    ...
    onSync: async (isSilent) => {
      ...
      } else if (['remove_bg_keys', 'cutout_pro_keys', 'gemini_keys', 'ilovepdf_keys', 'cloudconvert_keys', 'convertapi_keys'].includes(activeTab)) {
  ...
  const handleAddNewApiKey = async (e: React.FormEvent) => {
    ...
    // Deep Real-time Test for ConvertAPI before adding
    if (currentServiceName === 'convertapi') {
      showBanner('info', 'កំពុងតេស្តបញ្ជាក់សុពលភាពផ្ទាល់ជាមួយ ConvertAPI v2 Server...');
      const testRes = await testConvertApiKeyRealtime(newKeyString.trim());
      if (!testRes.success) {
        showBanner('error', `❌ មិនអាចរក្សាទុកបានទេ៖ ${testRes.message}`);
        setIsAddingKey(false);
        return;
  ...
  const handleTestSingleKey = async (id: number) => {
    ...
    try {
      ...
      if (currentServiceName === 'convertapi') {
        const kObj = apiKeys.find((k) => k.id === id);
        const kStr = kObj?.api_key || '';
        showBanner('info', `កំពុងតេស្ត ConvertAPI Key "${kObj?.key_label || ''}" ផ្ទាល់...`);
        const testRes = await testConvertApiKeyRealtime(kStr);
        try {
          await adminApi.testApiKey(id);
        } catch (_) {}

        if (testRes.success) {
          showBanner('success', `✅ Key "${kObj?.key_label || 'ConvertAPI'}" ${testRes.message}`);
        } else {
          showBanner('error', `❌ Key "${kObj?.key_label || 'ConvertAPI'}" ${testRes.message}`);
        }
        await loadApiKeys(currentServiceName, true);
        setTestingKeyId(null);
        return;
  ...
  const handleSyncAllKeys = async () => {
    ...
    try {
      ...
      if (currentServiceName === 'convertapi') {
        ...
        const updated = await Promise.all(
          apiKeys.map(async (k) => {
            const res = await testConvertApiKeyRealtime(k.api_key);
            if (res.success) activeCount++;
            else invalidCount++;
            ...
            return {
              ...
              last_status: res.status,
              free_calls: res.credits !== undefined ? res.credits : k.free_calls,
              credits: res.credits !== undefined ? res.credits : k.credits,
              last_checked_at: `${new Date().toISOString().replace('T', ' ').substring(0, 16)} (${res.latencyMs}ms)`,
            };
          })
        );
        ...
        await loadApiKeys(currentServiceName, true);
        showBanner('success', `បានធ្វើសមកាលកម្ម ConvertAPI Keys ទាំងអស់៖ ✅ ${activeCount} សកម្ម, ❌ ${invalidCount} មិនដំណើរការ`);
        setSyncingAllKeys(false);
        return;
      }
    ...
    <div className="admin-page-container fade-in" style={{ paddingBottom: '60px' }}>
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '6px', background: 'var(--surface-subtle, #f1f5f9)', padding: '6px', borderRadius: '14px', flexWrap: 'wrap' }}>
          ...
          <button
            onClick={() => setActiveTab('convertapi_keys')}
            style={{
              display: 'inline-flex',
              alignItems: 'center',
              gap: '8px',
              ...
              cursor: 'pointer',
              transition: 'all 0.2s ease',
              background: activeTab === 'convertapi_keys' ? '#fff' : 'transparent',
              color: activeTab === 'convertapi_keys' ? '#0284C7' : 'var(--text-secondary)',
              boxShadow: activeTab === 'convertapi_keys' ? '0 4px 12px rgba(0,0,0,0.06)' : 'none',
            }}
          >
            <Code2 size={15} color={activeTab === 'convertapi_keys' ? '#0284C7' : undefined} />
            <span>ConvertAPI ({convertApiCount})</span>
          </button>
        </div>
      </div>
      ...
      {/* ========================================================================= */}
      {/* ========================================================================= */}
      {/* 3. AI API KEYS POOL MANAGEMENT TAB (Remove.bg, Cutout.pro, Google Gemini, iLovePDF, CloudConvert & ConvertAPI) */}
      {/* ========================================================================= */}
      {(activeTab === 'remove_bg_keys' || activeTab === 'cutout_pro_keys' || activeTab === 'gemini_keys' || activeTab === 'ilovepdf_keys' || activeTab === 'cloudconvert_keys' || activeTab === 'convertapi_keys') && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          <div
            ...
            style={{
              ...
              background: activeTab === 'gemini_keys'
                ...
                : activeTab === 'convertapi_keys'
                ? 'linear-gradient(135deg, rgba(2, 132, 199, 0.08), rgba(6, 182, 212, 0.08))'
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              ...
              >
                {activeTab === 'gemini_keys' ? <Bot size={22} /> : activeTab === 'cutout_pro_keys' ? <Wand2 size={22} /> : activeTab === 'ilovepdf_keys' ? <FileText size={22} /> : activeTab === 'cloudconvert_keys' ? <Zap size={22} /> : activeTab === 'convertapi_keys' ? <Code2 size={22} /> : <Sparkles size={22} />}
              </span>
              <div>
                <h3 style={{ margin: 0, fontSize: '16px', fontWeight: 800, color: 'var(--text-primary)' }}>
                  {activeTab === 'gemini_keys'
                    ...
                    : activeTab === 'convertapi_keys'
                    ? 'ConvertAPI v2 Pool (Failover Engine for PDF to Word .docx)'
                ...
                <p style={{ margin: '2px 0 0 0', fontSize: '12px', color: 'var(--text-muted)' }}>
                  {activeTab === 'gemini_keys'
                    ...
                    : activeTab === 'convertapi_keys'
                    ? 'ConvertAPI ផ្តល់ជូន Free 1,500 Seconds / គណនីសម្រាប់បម្លែង PDF ទៅ Word (.docx) ដោយស្វ័យប្រវត្តិតាម Auto-Failover នៅពេល CloudConvert អស់ Credits។'
            ...
            >
              <span>
                {activeTab === 'gemini_keys'
                  ...
                  : activeTab === 'convertapi_keys'
                  ? 'យក API Secret ពី ConvertAPI.com'
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '16px' }}>
            ...
            {activeTab === 'gemini_keys' ? (
              ...
              <StatCard
                title="ConvertAPI Seconds នៅសល់"
                value={`${(apiKeyStats.total_credits || apiKeyStats.total_free_calls || 0).toLocaleString()} Secs`}
                subtitle="Conversion Seconds សម្រាប់ Failover"
                icon={<Code2 size={22} color="#0284C7" />}
              />
            ) : (
              <StatCard
            ...
            <StatCard
              title={activeTab === 'gemini_keys' ? 'Daily Limit (RPD) នៅសល់' : activeTab === 'cutout_pro_keys' ? 'HD Photo Credits' : activeTab === 'ilovepdf_keys' ? 'ឯកសារអាចបម្លែងបាន' : activeTab === 'cloudconvert_keys' || activeTab === 'convertapi_keys' ? 'ការបម្លែងថ្ងៃនេះ' : 'Full-Res Credits'}
              value={activeTab === 'gemini_keys' ? `${(apiKeyStats.total_daily_remaining ?? (apiKeyStats.active_keys * 1500)).toLocaleString()} / ${(apiKeyStats.total_daily_limit || (apiKeyStats.active_keys * 1500)).toLocaleString()}` : activeTab === 'ilovepdf_keys' ? `~${(apiKeyStats.total_free_calls || 250).toLocaleString()} Files` : activeTab === 'cloudconvert_keys' || activeTab === 'convertapi_keys' ? `${apiKeyStats.total_daily_used || 0} លើក` : `${apiKeyStats.total_credits}`}
              subtitle={activeTab === 'gemini_keys' ? `ប្រើប្រាស់ថ្ងៃនេះ៖ ${apiKeyStats.total_daily_used || 0} លើក` : activeTab === 'ilovepdf_keys' ? 'PDF ទៅ Word .docx' : activeTab === 'cloudconvert_keys' ? 'ម៉ាស៊ីន Office Engine v2' : activeTab === 'convertapi_keys' ? 'ម៉ាស៊ីន Failover v2' : 'កាត់រូបច្បាស់ High-Res'}
              icon={activeTab === 'cloudconvert_keys' ? <FileText size={22} color="#EA580C" /> : activeTab === 'convertapi_keys' ? <Code2 size={22} color="#0284C7" /> : activeTab === 'ilovepdf_keys' ? <FileText size={22} color="#F59E0B" /> : <Sparkles size={22} color="#F59E0B" />}
            />
            <StatCard
          ...
          >
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              ...
              >
                <Plus size={16} />
                <span>+ បន្ថែម {activeTab === 'gemini_keys' ? 'Google Gemini' : activeTab === 'cutout_pro_keys' ? 'Cutout.pro' : activeTab === 'ilovepdf_keys' ? 'iLovePDF' : activeTab === 'cloudconvert_keys' ? 'CloudConvert' : activeTab === 'convertapi_keys' ? 'ConvertAPI' : 'Remove.bg'} Key ថ្មី</span>
              </button>
            </div>
          </div>
                ...
                <thead>
                  <tr>
                    ...
                    <th style={{ textAlign: 'center' }}>
                      {activeTab === 'gemini_keys' ? 'Free Quota (RPM)' : activeTab === 'cutout_pro_keys' ? 'Free Credits' : activeTab === 'ilovepdf_keys' ? 'Credits & Files នៅសល់' : activeTab === 'cloudconvert_keys' ? 'Credits (Mins)' : activeTab === 'convertapi_keys' ? 'Credits (Seconds)' : 'Free Calls / ខែ'}
                    </th>
                    <th style={{ textAlign: 'center' }}>
                      {activeTab === 'gemini_keys' ? 'Daily Limit (RPD)' : activeTab === 'cutout_pro_keys' ? 'Paid / Total Credits' : activeTab === 'ilovepdf_keys' ? 'Conversion Type' : activeTab === 'cloudconvert_keys' || activeTab === 'convertapi_keys' ? 'Conversion Engine' : 'Full-Res Credits'}
                    </th>
                          ...
                          <tr key={k.id}>
                            ...
                            <td style={{ textAlign: 'center' }}>
                              {activeTab === 'ilovepdf_keys' ? (
                                ...
                                >
                                  {(k.credits || k.free_calls || 0).toLocaleString()} Secs
                                </span>
                              ) : (
                                <span
                                  style={{
                            ...
                            <td style={{ textAlign: 'center' }}>
                              {activeTab === 'gemini_keys' ? (
                                ...
                                <div style={{ display: 'inline-flex', flexDirection: 'column', alignItems: 'center', gap: '4px' }}>
                                  ...
                                  <div style={{ fontSize: '10.5px', color: 'var(--text-muted)' }}>
                                    ប្រើថ្ងៃនេះ៖ <strong style={{ color: (k.daily_requests_used || 0) > 0 ? '#0284C7' : 'var(--text-muted)' }}>{k.daily_requests_used || 0}</strong> លើក
                                  </div>
                                </div>
                              ) : (
                  ...
                  {activeTab === 'ilovepdf_keys' ? (
                    ...
                    <div className="form-group" style={{ marginBottom: '20px' }}>
                      ...
                      <input
                        type="text"
                        className="form-input"
                        placeholder={activeTab === 'gemini_keys' ? 'ឧ. AIzaSyD...' : activeTab === 'cloudconvert_keys' ? 'ឧ. eyJ0eXAiOiJKV1QiLCJhbGciOiJSUzI1Ni...' : activeTab === 'convertapi_keys' ? 'ឧ. secret_xxxxxxxxxxxxxxxxxxxx' : 'ឧ. LM9UPg8HqRKeZ89FeM2hhaCR'}
                        value={newKeyString}
                        onChange={(e) => setNewKeyString(e.target.value)}
                        required
                        style={{ fontFamily: 'monospace' }}
                      ...
                      <span style={{ fontSize: '11.5px', color: 'var(--text-muted)', marginTop: '4px', display: 'block' }}>
                        {activeTab === 'gemini_keys'
                          ...
                          : activeTab === 'convertapi_keys'
                          ? 'ប្រព័ន្ធនឹងធ្វើការ Test ផ្ទៀងផ្ទាត់ផ្ទាល់ជាមួយ ConvertAPI v2 Server (user info) ភ្លាមៗមុនពេល Save។'
