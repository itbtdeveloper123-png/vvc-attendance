...
set_exception_handler(function($exception) {
    ...
    echo json_encode(['success' => false, 'status' => 'error', 'message' => 'Server Error: ' . $exception->getMessage()]);
    exit;
});
...
try {
    ...
    case 'summarize_meeting':
        ...
        try {
            ...
            if (!$mid) {
                apiResponse(['success' => false, 'message' => 'Missing meeting ID']);
            }
            ...
            if (!$meeting) {
                apiResponse(['success' => false, 'message' => 'រកមិនឃើញទិន្នន័យកិច្ចប្រជុំឡើយ (Meeting not found)']);
            }
            ...
            if ($summaryText === '') {
                ...
                if ($summaryText === '' && $groqKey !== '') {
                    $groqModels = ['openai/gpt-oss-120b', 'qwen/qwen3.6-27b', 'llama-3.3-70b-versatile', 'llama-3.1-8b-instant'];
                    foreach ($groqModels as $groqModel) {
                        try {
                            ...
                            if ($groqRaw && $groqHttp === 200) {
                                ...
                                $lastError = 'Groq ' . $groqModel . ': ' . ($errDec['error']['message'] ?? "HTTP {$groqHttp}");
                            }
                        } catch (Throwable $gre) {
                ...
                // 3c. Fallback to GitHub Models (GPT-4o-mini)
                if ($summaryText === '' && $githubToken !== '') {
                    try {
                        $ghUrl = 'https://models.inference.ai.azure.com/chat/completions';
                        $ch = curl_init($ghUrl);
                        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
                        curl_setopt($ch, CURLOPT_POST, true);
                        curl_setopt($ch, CURLOPT_HTTPHEADER, [
                        ...
                        curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
                        $ghRaw = curl_exec($ch);
                        $ghHttp = curl_getinfo($ch, CURLINFO_HTTP_CODE);
                        curl_close($ch);

                        if ($ghRaw && $ghHttp === 200) {
                            $ghDec = json_decode($ghRaw, true);
                            if (!empty($ghDec['choices'][0]['message']['content'])) {
                                $summaryText = trim($ghDec['choices'][0]['message']['content']);
                                $usedProvider = 'github-models';
                                $usedModel = 'gpt-4o-mini';
                            }
                        }
                    } catch (Throwable $ghe) {
                        $lastError = 'GitHub Models: ' . $ghe->getMessage();
                    }
                }
            }
            ...
            apiResponse([
                'success' => false,
                'status' => 'error',
                'message' => 'កំហុសបច្ចេកទេស AI: ' . $mainEx->getMessage()
            ]);
        }
        break;
...
២) ការកំណត់ជំពូកផលិតផល (Category)៖
   - ត្រូវកំណត់ឱ្យត្រឹមត្រូវតាមប្រភេទផលិតផលជាក់ស្តែង ដូចជា៖
     * ដបទឹក និងកែវរក្សាកម្តៅ (Water Bottles, Tumblers, Thermos & Vacuum Flasks)
...
៣) ព័ត៌មានលម្អិតនៃផលិតផល៖
   - ចំពោះដបទឹក កែវទឹក ឬ Tumbler / Thermos / Vacuum Flask / Sports Bottle៖
     * ingredients_summary: ប្តូរមកជាការបញ្ជាក់ «សម្ភារៈ និងលក្ខណៈបច្ចេកទេស» (Materials & Specifications) ឱ្យបានច្បាស់លាស់ ដូចជា កម្រិតដែកអ៊ីណុក (SUS 304 / 316 Food Grade), ប្លាស្ទិក Tritan (BPA-Free), ជ័រ Silicone, Double-wall vacuum insulation, ចំណុះ (Volume/Capacity ដូចជា 500ml, 750ml, 1L, etc.) និងប្រព័ន្ធគម្រប Leak-proof។
     * usage (array): របៀបប្រើប្រាស់ និងការថែទាំដបទឹកឱ្យប្រើប្រាស់បានយូរ (ការលាងសម្អាតដំបូងមុនប្រើជាមួយទឹកក្តៅឧណ្ហៗ, ការសម្អាតប្រចាំថ្ងៃ, ការចៀសវាងការដាក់ក្នុង Microwave ឬ Freezer, របៀបសម្អាតកៅស៊ូ Silicone seal ការពារជាប់ក្លិន...)។
     * benefits (array): លក្ខណៈពិសេសនៃការរក្សាសីតុណ្ហភាព (ម៉ោងរក្សាត្រជាក់ Keep Cold, ម៉ោងរក្សាកម្តៅ Keep Hot), មិនឡើងញើសខាងក្រៅ (Sweat-free), គ្មានសារធាតុ BPA/Lead Free, ធន់នឹងការប៉ះទង្គិច, ងាយស្រួលយួរតាមខ្លួន...
     * warnings (array): ការប្រុងប្រយ័ត្នចំពោះដបទឹក (ហាមដាក់ក្នុង Microwave, ហាមដាក់ក្នុង Freezer, ចៀសវាងដាក់ភេសជ្ជៈមានហ្គាសកាបូណាត ព្រោះអាចបណ្តាលឱ្យឡើងសម្ពាធបើកគម្របមិនរួច ឬផ្ទុះគម្រប, ចៀសវាងត្រាំទឹកដោះគោ ឬទឹកផ្លែឈើជូរយូរម៉ោងដើម្បីការពារបាក់តេរី...)។
   - ចំពោះផលិតផលទូទៅផ្សេងទៀត៖
     ...
     * warnings (array): ការប្រុងប្រយ័ត្ន និងសុវត្ថិភាព។
...
៤) ទម្រង់ឆ្លើយតប (Output Format)៖
   - ត្រូវឆ្លើយតបតែទម្រង់ JSON object {...} ត្រឹមត្រូវមួយគត់ (keys: product_name, brand, country_of_origin, country_flag_emoji, category, usage, benefits, warnings, ingredients_summary, price_range_usd, summary) ដោយមិនប្រើ Markdown code blocks (```) ឡើយ។';
    ...
    case 'product_chat':
        ...
        $systemInstructions = "អ្នកជា AI Chatbot Assistant ឆ្លាតវៃ និងមានជំនាញវិជ្ជាជីវៈខ្ពស់ សម្រាប់ផ្តល់ប្រឹក្សា និងវិភាគផលិតផលទូទៅ ដបទឹក និងកែវរក្សាកម្តៅ (Water Bottles, Tumblers, Thermos, Vacuum Flasks) គ្រឿងសំអាង ចំណីអាហារ និងឱសថ ជូនប្រជាជនកម្ពុជា។\n"
            ...
            . "២. ឆ្លើយផ្អែកលើព័ត៌មានផលិតផលខាងលើ និងចំណេះដឹងវិទ្យាសាស្ត្រពិតប្រាកដ។ បើសួរអំពីដបទឹក ឬកែវរក្សាកម្តៅ សូមពន្យល់អំពីសម្ភារៈ (ដែកអ៊ីណុក SUS 304/316, BPA-Free Tritan), ម៉ោងរក្សាកម្តៅ-ត្រជាក់, ការលាងសម្អាត, ការហាមឃាត់ដាក់ក្នុង Microwave/Freezer និងការចៀសវាងដាក់ភេសជ្ជៈមានហ្គាស។\n"
    ...
    case 'get_polls':
        try {
            ...
            $findUser = function($idOrEid) use (&$users_map) {
                $str = trim((string)$idOrEid);
                if ($str === '') return null;
                $clean = ltrim($str, '0');
            ...
            if ($result && $result instanceof mysqli_result) {
                while ($row = $result->fetch_assoc()) {
                    ...
                    if ($poll_id > 0) {
                        $cand_q = $mysqli->query("
                            SELECT c.id, c.employee_id, c.category,
                        ...
                        if ($cand_q && $cand_q instanceof mysqli_result) {
                            while ($c_row = $cand_q->fetch_assoc()) {
                                ...
                                $u_info = $findUser($c_eid);
                                $c_row['name'] = $u_info['name'] ?? $c_eid;
                                $c_row['department'] = $u_info['department'] ?? $u_info['position'] ?? 'បុគ្គលិក';
                                $c_row['position'] = $u_info['position'] ?? 'បុគ្គលិក';
                                $c_row['photo_url'] = $u_info['photo_url'] ?? $u_info['avatar'] ?? $u_info['photo'] ?? '';
                        ...
                        if (empty($candidates)) {
                            ...
                            if (!empty($emp_ids)) {
                                foreach ($emp_ids as $c_eid) {
                                    $u_info = $findUser($c_eid);
                                    $v_cnt_q = $mysqli->query("
                                        SELECT COUNT(*) AS cnt FROM poll_votes v
                                        WHERE v.poll_id = $poll_id AND (v.candidate_employee_id = '$c_eid' OR v.candidate_id = '$c_eid' OR TRIM(LEADING '0' FROM v.candidate_employee_id) = TRIM(LEADING '0' FROM '$c_eid'))
                                    ");
                                    $v_cnt = ($v_cnt_q && $v_cnt_row = $v_cnt_q->fetch_assoc()) ? (int)($v_cnt_row['cnt'] ?? 0) : 0;
                                    $candidates[] = [
                                        'id' => 0,
                                        'poll_id' => $poll_id,
                                        'employee_id' => $c_eid,
                                        'name' => $u_info['name'] ?? $c_eid,
                                        'department' => $u_info['department'] ?? $u_info['position'] ?? 'បុគ្គលិក',
                                        'position' => $u_info['position'] ?? 'បុគ្គលិក',
                                        'category' => $row['location'] ?? 'Head Office',
                                        'photo_url' => $u_info['photo_url'] ?? $u_info['avatar'] ?? $u_info['photo'] ?? '',
                                        'votes_count' => $v_cnt,
                                    ];
                                }
                            }
                    ...
                    if (!empty($voter_eid)) {
                        ...
                        $v_chk = $mysqli->query("SELECT pv.candidate_id, COALESCE(pc.employee_id, pv.candidate_employee_id, CAST(pv.candidate_id AS CHAR)) as cand_emp_id 
                                                 FROM poll_votes pv 
                    ...
                    $audit_q = $mysqli->query("
                        SELECT v.id, v.poll_id, v.voter_employee_id, v.candidate_id, v.candidate_employee_id, v.voted_at
                        FROM poll_votes v
                    ...
                    if ($audit_q && $audit_q instanceof mysqli_result) {
                        while ($a_row = $audit_q->fetch_assoc()) {
                            $v_eid = (string)$a_row['voter_employee_id'];
                            $c_eid = (string)($a_row['candidate_employee_id'] ?? '');
                            if (empty($c_eid) || $c_eid === '0') {
                                $cand_num_id = (int)$a_row['candidate_id'];
                                if ($cand_num_id > 0) {
                                    $c_lookup = $mysqli->query("SELECT employee_id FROM poll_candidates WHERE id = $cand_num_id LIMIT 1");
                                    if ($c_lookup && $c_lr = $c_lookup->fetch_assoc()) {
                                        $c_eid = (string)$c_lr['employee_id'];
                                    } else {
                                        $c_eid = (string)$cand_num_id;
                                    }
                                }
                            }
                            $u_v = $findUser($v_eid);
                            ...
                            $audit_list[] = [
                                'voter_id' => $v_eid,
                                'candidate_id' => $c_eid,
                                'voter_name' => $u_v['name'] ?? $v_eid,
                                'candidate_name' => $u_c['name'] ?? $c_eid,
    ...
    case 'update_poll':
        try {
            ...
            if ($stmt->execute()) {
                ...
                if (isset($_POST['candidates'])) {
                    ...
                    if (is_array($cands_input)) {
                        foreach ($cands_input as $cand) {
                            ...
                            $cand_cat = $location;
                            if (is_array($cand)) {
                                $cand_emp_id = $cand['employee_id'] ?? $cand['id'] ?? '';
                                $cand_cat = $cand['category'] ?? $location;
                            } else {
                                $cand_emp_id = (string)$cand;
                            }
                ...
                error_log("SAVE_POLL: Successfully saved poll ID: $saved_poll_id");
                apiResponse(['success' => true, 'message' => 'រក្សាទុកការបោះឆ្នោតបានជោគជ័យ', 'poll_id' => $saved_poll_id]);
            } else {
    ...
    case 'get_poll_results':
        ...
        // Build comprehensive in-memory user lookup map using SELECT *
        $users_map = [];
        $u_all = $mysqli->query("SELECT * FROM users");
        if ($u_all && $u_all instanceof mysqli_result) {
            while ($u = $u_all->fetch_assoc()) {
                ...
                $u_id = trim((string)($u['id'] ?? ''));
                $u_clean = ltrim($u_eid, '0');
                $u_digits = preg_replace('/\D/', '', $u_eid);
                $u_clean_digits = ltrim($u_digits, '0');
                ...
                if ($u_digits !== '') $users_map[$u_digits] = $u;
                if ($u_clean_digits !== '') $users_map[$u_clean_digits] = $u;
                if ($u_id !== '') {
                    $users_map['id_' . $u_id] = $u;
                    $users_map[$u_id] = $u;
                }
            }
            $u_all->free();
        ...
        $findUser = function($idOrEid) use (&$users_map) {
            ...
            $clean_digits = ltrim($digits, '0');

            if (isset($users_map[$str])) return $users_map[$str];
            if ($clean !== '' && isset($users_map[$clean])) return $users_map[$clean];
            if ($digits !== '' && isset($users_map[$digits])) return $users_map[$digits];
            if ($clean_digits !== '' && isset($users_map[$clean_digits])) return $users_map[$clean_digits];
            if ($clean !== '' && isset($users_map[str_pad($clean, 4, '0', STR_PAD_LEFT)])) return $users_map[str_pad($clean, 4, '0', STR_PAD_LEFT)];
            if ($clean !== '' && isset($users_map[str_pad($clean, 3, '0', STR_PAD_LEFT)])) return $users_map[str_pad($clean, 3, '0', STR_PAD_LEFT)];
            if (isset($users_map['id_' . $str])) return $users_map['id_' . $str];
            return null;
        ...
        $stmt = $mysqli->prepare("SELECT p.* FROM poll_events p ORDER BY p.created_at DESC");
        if (!$stmt) {
            apiResponse(['success' => false, 'message' => 'Database error: ' . $mysqli->error]);
            break;
        }
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            $candidates = [];
            $existing_emp_ids = [];
            $cand_q = $mysqli->query("SELECT c.id, c.employee_id, c.category FROM poll_candidates c WHERE c.poll_id = $poll_id ORDER BY c.id ASC");
            if ($cand_q && $cand_q instanceof mysqli_result) {
                while ($c_row = $cand_q->fetch_assoc()) {
                    $c_eid = (string)$c_row['employee_id'];
                    ...
                    $u_info = $findUser($c_eid);
                    $c_name = $u_info['name'] ?? $c_eid;
                    $c_dept = $u_info['department'] ?? $u_info['position'] ?? 'បុគ្គលិក';
                    $c_cat = $c_row['category'] ?? $u_info['branch'] ?? $row['location'] ?? 'Head Office';
            ...
            // 2. Fallback from allowed_employee_ids if candidates are not in poll_candidates
            $allowed_ids = [];
            ...
            if (is_array($allowed_ids) && count($allowed_ids) > 0) {
                foreach ($allowed_ids as $cand_eid) {
                    ...
                    $u_info = $findUser($cand_eid);

                    $candidates[] = [
                        'id'          => 0,
                        'name'        => $u_info['name'] ?? $cand_eid,
            ...
            if ($audit_q && $audit_q instanceof mysqli_result) {
                while ($a_row = $audit_q->fetch_assoc()) {
                    ...
                    $c_id = (string)($a_row['candidate_employee_id'] ?? '');
                    if (empty($c_id) || $c_id === '0') {
                        $cand_num_id = (int)$a_row['candidate_id'];
                        if ($cand_num_id > 0) {
                            $c_lookup = $mysqli->query("SELECT employee_id FROM poll_candidates WHERE id = $cand_num_id LIMIT 1");
                            if ($c_lookup && $c_lr = $c_lookup->fetch_assoc()) {
                                $c_id = (string)$c_lr['employee_id'];
                            } else {
                                $c_id = (string)$cand_num_id;
                            }
                        }
                    }

                    $u_v = $findUser($v_id);
                    $v_name = $u_v['name'] ?? $v_id;
                    $v_dept = $u_v['department'] ?? $u_v['position'] ?? $u_v['branch'] ?? 'បុគ្គលិក';

                    $u_c = $findUser($c_id);
                    $c_name = $u_c['name'] ?? $c_id;

                    $audit_list[] = [
                        'voter_id'           => $v_id,
                        'voter_name'         => !empty($v_name) ? $v_name : $v_id,
                        'voter_department'   => $v_dept,
                        'candidate_id'       => $c_id,
                        'candidate_name'     => !empty($c_name) ? $c_name : $c_id,
                        'time'               => !empty($a_row['voted_at']) ? date('d/m/Y H:i', strtotime($a_row['voted_at'])) : '',
                    ];
                }
                $audit_q->free();
            ...
            // 4. Calculate candidate vote counts directly matching audit list
            $total_audit_votes = count($audit_list);
            foreach ($candidates as &$cand) {
                $cand_eid = (string)$cand['employee_id'];
                $cand_cid = (string)$cand['id'];
                $cand_clean = ltrim($cand_eid, '0');
                $cand_nm = trim((string)$cand['name']);

                $v_cnt = 0;
                foreach ($audit_list as $a) {
                    $a_cid = (string)$a['candidate_id'];
                    $a_clean = ltrim($a_cid, '0');
                    $a_nm = trim((string)$a['candidate_name']);

                    if ($a_cid === $cand_eid || ($cand_clean !== '' && $a_clean === $cand_clean) || ($cand_cid !== '0' && $a_cid === $cand_cid)) {
                        $v_cnt++;
                    } else if ($cand_nm !== '' && $a_nm !== '' && strcasecmp($cand_nm, $a_nm) === 0) {
                        $v_cnt++;
                    }
                }
                $cand['votes'] = $v_cnt;
                $cand['votes_count'] = $v_cnt;
                $cand['percentage'] = $total_audit_votes > 0 ? round(($v_cnt / $total_audit_votes) * 100, 1) : 0;
            }
            unset($cand);

            // Also check for any candidate in audit_list not in candidates array
            foreach ($audit_list as $a) {
                $a_cid = (string)$a['candidate_id'];
                $a_nm = (string)$a['candidate_name'];
                $found = false;
                foreach ($candidates as $c) {
                    if ((string)$c['employee_id'] === $a_cid || (ltrim((string)$c['employee_id'], '0') !== '' && ltrim((string)$c['employee_id'], '0') === ltrim($a_cid, '0')) || $c['name'] === $a_nm) {
                        $found = true;
                        break;
                    }
                }
                if (!$found && $a_cid !== '') {
                    $u_info = $findUser($a_cid);
                    $v_cnt = 0;
                    foreach ($audit_list as $inner_a) {
                        if ($inner_a['candidate_id'] === $a_cid || (ltrim($inner_a['candidate_id'], '0') !== '' && ltrim($inner_a['candidate_id'], '0') === ltrim($a_cid, '0'))) $v_cnt++;
                    }
                    $candidates[] = [
                        'id'          => (int)$a_cid,
                        'name'        => $u_info['name'] ?? $a_nm,
                        'employee_id' => $a_cid,
                        'category'    => $row['location'] ?? 'Head Office',
                        'department'  => $u_info['department'] ?? $u_info['position'] ?? 'បុគ្គលិក',
                        'votes'       => $v_cnt,
                        'votes_count' => $v_cnt,
                        'percentage'  => $total_audit_votes > 0 ? round(($v_cnt / $total_audit_votes) * 100, 1) : 0,
                    ];
                }
            }
            ...
            $row['voter_audit_list'] = $audit_list;
            $row['total_votes'] = $total_audit_votes;
            $polls[] = $row;
        }
        $stmt->close();
        apiResponse(['success' => true, 'data' => $polls]);
    ...
    case 'get_active_polls':
        ...
        if ($res) {
            while ($row = $res->fetch_assoc()) {
                ...
                $voter_eid_clean = ltrim($eid, '0');
                $vote_check_sql = "
                    SELECT pv.candidate_id, COALESCE(pc.employee_id, pv.candidate_employee_id, CAST(pv.candidate_id AS CHAR)) as cand_emp_id 
                    FROM poll_votes pv 
                    LEFT JOIN poll_candidates pc ON (pv.candidate_id = pc.id OR CAST(pv.candidate_id AS CHAR) = pc.employee_id OR TRIM(LEADING '0' FROM CAST(pv.candidate_id AS CHAR)) = TRIM(LEADING '0' FROM pc.employee_id))
                    WHERE pv.poll_id = $poll_id 
                      AND (pv.voter_employee_id = '$voter_eid_esc' OR TRIM(LEADING '0' FROM pv.voter_employee_id) = '$voter_eid_clean') 
                    ORDER BY pv.id DESC LIMIT 1
                ";
                $v_chk_q = $mysqli->query($vote_check_sql);
                if ($v_chk_q && $vote_res = $v_chk_q->fetch_assoc()) {
                    $row['has_voted'] = true;
                ...
                $existing_emp_ids = [];
                $cand_q = $mysqli->query("
                    SELECT pc.id, pc.employee_id, pc.category,
                           (SELECT COUNT(*) FROM poll_votes pv 
                            WHERE pv.poll_id = pc.poll_id 
                              AND (
                                pv.candidate_id = pc.id 
                                OR pv.candidate_employee_id = pc.employee_id
                                OR TRIM(LEADING '0' FROM pv.candidate_employee_id) = TRIM(LEADING '0' FROM pc.employee_id)
                                OR CAST(pv.candidate_id AS CHAR) = pc.employee_id
                                OR TRIM(LEADING '0' FROM CAST(pv.candidate_id AS CHAR)) = TRIM(LEADING '0' FROM pc.employee_id)
                                OR pv.candidate_id IN (SELECT id FROM poll_candidates WHERE (employee_id = pc.employee_id OR TRIM(LEADING '0' FROM employee_id) = TRIM(LEADING '0' FROM pc.employee_id)))
                              )
                           ) AS votes_count
                ...
                if ($cand_q && $cand_q instanceof mysqli_result) {
                    while ($c_row = $cand_q->fetch_assoc()) {
                        ...
                        $c_emp_id = (string)$c_row['employee_id'];
                        $existing_emp_ids[] = $c_emp_id;
                        $existing_emp_ids[] = ltrim($c_emp_id, '0');

                        $u_info = $findUser($c_emp_id);
                        $c_name = $u_info['name'] ?? $c_emp_id;
                        $c_dept = $c_row['category'] ?? $u_info['branch'] ?? $u_info['department'] ?? 'Head Office';
                        $c_photo = $u_info['photo_url'] ?? $u_info['avatar'] ?? $u_info['photo'] ?? '';
                        ...
                        $candidates[] = [
                            ...
                            'name'        => $c_name,
                            'category'    => $c_dept,
                            'department'  => $u_info['department'] ?? $u_info['position'] ?? $c_dept,
                            'photo_url'   => $c_photo,
                            'votes_count' => $v_cnt,
                            'votes'       => $v_cnt,
                        ];
                ...
                // Also include candidates from allowed_employee_ids who are not yet in poll_candidates table
                if (is_array($allowed_arr) && count($allowed_arr) > 0) {
                    foreach ($allowed_arr as $idx => $cand_emp_id) {
                        $cand_emp_id = (string)$cand_emp_id;
                        $cand_clean = ltrim($cand_emp_id, '0');
                        if (in_array($cand_emp_id, $existing_emp_ids) || ($cand_clean !== '' && in_array($cand_clean, $existing_emp_ids))) {
                            continue;
                        }
                        $u_info = $findUser($cand_emp_id);
                        ...
                        $vc_res = $mysqli->query("
                            ...
                            WHERE pv.poll_id = $poll_id 
                              AND (
                                pv.candidate_employee_id = '$cand_emp_id'
                                OR TRIM(LEADING '0' FROM pv.candidate_employee_id) = TRIM(LEADING '0' FROM '$cand_emp_id')
                                OR CAST(pv.candidate_id AS CHAR) = '$cand_emp_id'
                                OR TRIM(LEADING '0' FROM CAST(pv.candidate_id AS CHAR)) = TRIM(LEADING '0' FROM '$cand_emp_id')
                              )
                        ");
                        ...
                        $candidates[] = [
                            ...
                            'name'        => $u_info['name'] ?? $cand_emp_id,
                            'category'    => $u_info['branch'] ?? $row['location'] ?? 'Head Office',
                            'photo_url'   => $u_info['photo_url'] ?? $u_info['avatar'] ?? '',
                            'votes_count' => $v_cnt,
                            'votes'       => $v_cnt,
                        ];
                    }
                ...
                $audit_q = $mysqli->query("
                    SELECT 
                        ...
                        uv.department as voter_department,
                        COALESCE(uc.name, uc2.name, pc.employee_id, pv.candidate_employee_id, CAST(pv.candidate_id AS CHAR)) as candidate_name,
                        pv.voted_at
                    FROM poll_votes pv
                    LEFT JOIN users uv ON (pv.voter_employee_id = uv.employee_id OR TRIM(LEADING '0' FROM pv.voter_employee_id) = TRIM(LEADING '0' FROM uv.employee_id))
                    LEFT JOIN poll_candidates pc ON (pv.candidate_id = pc.id OR (pc.employee_id = CAST(pv.candidate_id AS CHAR) OR TRIM(LEADING '0' FROM pc.employee_id) = TRIM(LEADING '0' FROM CAST(pv.candidate_id AS CHAR))))
                    LEFT JOIN users uc ON (pc.employee_id = uc.employee_id OR TRIM(LEADING '0' FROM pc.employee_id) = TRIM(LEADING '0' FROM uc.employee_id))
                    LEFT JOIN users uc2 ON (CAST(pv.candidate_id AS CHAR) = uc2.employee_id OR TRIM(LEADING '0' FROM CAST(pv.candidate_id AS CHAR)) = TRIM(LEADING '0' FROM uc2.employee_id))
                    WHERE pv.poll_id = $poll_id
                    ORDER BY pv.id DESC
                ");
                if ($audit_q && $audit_q instanceof mysqli_result) {
                    while ($a_row = $audit_q->fetch_assoc()) {
                        $v_id = (string)$a_row['voter_employee_id'];
                        $c_id = (string)($a_row['candidate_employee_id'] ?? $a_row['candidate_id']);
                        $v_name = (string)($a_row['voter_name'] ?? '');
                        $c_name = (string)($a_row['candidate_name'] ?? '');

                        $u_v = $findUser($v_id);
                        if ($u_v && !empty($u_v['name'])) {
                            $v_name = $u_v['name'];
                        }

                        $u_c = $findUser($c_id);
                        if ($u_c && !empty($u_c['name'])) {
                            $c_name = $u_c['name'];
                        }
                ...
                $row['total_votes'] = count($voter_audit_list);
                $polls[] = $row;
            }
        }
