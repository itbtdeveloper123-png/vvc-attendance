...
class ForgetScanScreen extends StatefulWidget {
  const ForgetScanScreen({super.key});

  @override
  State<ForgetScanScreen> createState() => _ForgetScanScreenState();
}

class _ForgetScanScreenState extends State<ForgetScanScreen> {
  ...
  void _submit() async {
    ...
    final formData = {
      'forget_date': DateFormat('yyyy-MM-dd').format(_selectedDate),
    ...
    final result = await _apiService.submitRequest(
      'Forget-Attendance',
      formData,
    );
                    ...
                    Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          _buildLabelField(
                            "ប្រភេតដែលភ្លេច",
                            _buildDropdownKhmer(_forgetType, [
                              'Check-In',
                              'Check-Out',
                              'Both',
                            ], (v) => setState(() => _forgetType = v!)),
                          ),
                          const SizedBox(height: 20),
                          _buildLabelField(
                            "មូលហេតុ",
